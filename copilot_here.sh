...
# DO NOT EDIT BELOW THIS LINE - Use 'copilot_here --update-scripts' to update
# ============================================================================
...
EOF
      ...
      --update-scripts)
        ...
        # Extract just the script content (between markers)
        local start_marker="# ============================================================================"
        local end_marker="# END copilot_here shell functions"
        
        # Create backup
        cp "$shell_config" "${shell_config}.backup.$(date +%Y%m%d_%H%M%S)"
        echo "✅ Created backup: ${shell_config}.backup.$(date +%Y%m%d_%H%M%S)"
        
        # Check if markers exist in config
        if grep -q "$start_marker" "$shell_config" && grep -q "$end_marker" "$shell_config"; then
          # Replace between markers
          awk -v new_content="$(cat "$temp_script")" '
            /# ============================================================================/ { print new_content; skip=1; next }
            /# END copilot_here shell functions/ { skip=0; next }
            !skip
          ' "$shell_config" > "${shell_config}.tmp"
          mv "${shell_config}.tmp" "$shell_config"
          echo "✅ Scripts updated successfully!"
        else
          echo "⚠️  Markers not found in $shell_config"
          echo "Appending script to the end of file..."
          echo "" >> "$shell_config"
          cat "$temp_script" >> "$shell_config"
...
# ============================================================================
# END copilot_here shell functions
# ============================================================================
```
</details>
